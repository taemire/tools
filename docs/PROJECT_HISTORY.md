...
## 2026-02-17: 레거시 도구 정리 및 아카이빙

### 작업 내용
- **혼란스러운 디렉토리 구조 정리**: `md2pdf_v2` (Go 소스) 폴더의 이름이 `md2pdf_v2.bat` (배치 스크립트)와 충돌하여 혼란을 유발함.
- **아카이빙**: 사용되지 않는 레거시 도구들을 `_archive/` 디렉토리로 이동:
  - `tools/md2pdf_v2/` (Go Source): `gopdf` 기반의 실험적 구현체. 현재의 `html2pdf` 방식(Chromium)으로 완전히 대체됨.
  - `tools/md2html/` (v1): `md2html_v2`로 기능 이관 완료.
  - `tools/md2pdf/` (v1): 초기 실험 버전.
  - `tools/ref/`: 참조용 파일들.
- **문서화**: `README.md`를 전면 개편하여 활성(Active) 도구와 아카이브(Archived) 도구를 명확히 구분.

### 의사결정
- `md2pdf_v2.bat` / `md2pdf_v2.sh`를 공식적인 PDF 생성 엔트리 포인트로 확정.
- Go 기반의 네이티브 PDF 생성(`gopdf`)은 CSS Paged Media 지원 부족으로 인해 중단됨을 공식화.
